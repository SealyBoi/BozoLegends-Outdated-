...
public class shop {
	...
	private int currItemDisplayed;
	
	private int getCID() {
		return currItemDisplayed;
	}
	
	private void setCID(int item) {
		currItemDisplayed = item;
	}
	...
	public void enterShop(String shopName, JLabel itemName, JLabel itemCost, JLabel itemMod, String[] itemNames,
		...
		setCID(0);
		itemDisplayedCounter.setText((getCID() + 1) + "/" + numberOfItems);
		...
		if (itemsPurchased[0]) {
			...
			if (getCID() == player.getWeapon() && shopName.equals("weapon")
					|| getCID() == player.getArmor() && shopName.equals("armor")) {
	...
	public boolean purchaseItem(int[] itemPrices, boolean[] itemsPurchased, JButton purchaseItem, JButton equipItem) {
		if (itemPrices[getCID()] <= player.getGold()) {
			player.setGold(player.getGold() - itemPrices[getCID()]);
			itemsPurchased[getCID()] = true;
			purchaseItem.setVisible(false);
			equipItem.setVisible(true);
			equipItem.setText("Equip");
			return true;
	...
	public void equipItem(String shopName, String[] equippedItem, JButton equipItem, JLabel townItemEquipped,
		...
		equippedItem[0] = itemNames[getCID()];
		equippedItem[1] = "" + itemPrices[getCID()];
		equippedItem[2] = "" + itemMods[getCID()];
		equipItem.setText("Equipped");
		player.setDamage(itemMods[getCID()]);
		switch (shopName) {
		case "weapon":
			townItemEquipped.setText("Weapon: " + equippedItem[0]);
			player.setWeapon(getCID());
			break;
		case "armor":
			townItemEquipped.setText("Armor: " + equippedItem[0]);
			player.setArmor(getCID());
			break;
	...
	public void equipAmulet(String[] equippedAmulet, String[] amuletNames, int[] amuletMods, JButton equipAmulet,
		...
		equippedAmulet[0] = amuletNames[getCID()];
		equippedAmulet[1] = "" + amuletMods[getCID()];
		equipAmulet.setText("Equipped");
		player.setAmulet(getCID());
		switch (player.getAmulet()) {
		case 0:
			player.setAccuracy(player.getAccuracy() + Integer.parseInt(equippedAmulet[1]));
			break;
	...
	public void nextItem(String shopName, int numberOfItems, JLabel itemDisplayedCounter, JLabel itemName,
		...
		if (getCID() < numberOfItems - 1) {
			setCID(getCID() + 1);
			itemDisplayedCounter.setText((getCID() + 1) + "/" + numberOfItems);
			itemName.setText(itemNames[getCID()]);
			itemCost.setText(itemPrices[getCID()] + " Gold");
			switch (shopName) {
			case "weapon":
				itemMod.setText(
						(itemMods[getCID()] - 2) + "-" + (itemMods[getCID()] + 1) + " Damage");
				break;
			case "armor":
				itemMod.setText(itemMods[getCID()] + " Extra Health");
				break;
	...
	public void nextDrop(JLabel dropDisplayedCounter, JLabel dropName, JLabel dropDesc, String[] dropNames,
			...
			JButton equipDrop) {
		if (getCID() < numberOfDrops - 1) {
			setCID(getCID() + 1);
			dropDisplayedCounter.setText((getCID() + 1) + "/" + numberOfDrops);
			dropName.setText(dropNames[getCID()]);
			dropDesc.setText(dropDescs[getCID()]);
			if (getCID() >= numberOfDrops - 1) {
				nextDrop.setBackground(Color.darkGray);
			} // if there is no item after this one, show player they are at the end of the list
			previousDrop.setBackground(Color.green);
			if (dropsUnlocked[getCID()]) {
				if (player.getAmulet() == getCID()) {
